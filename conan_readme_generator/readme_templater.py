...
class BincraftersTemplater(object):
            ...
            try:
                ...
                try:
                    ...
                    if debug:
                        ...
                        print(
                            "No default options found in conanfile. Skipping.")
                    pass
                else:
    ...
    def get_recipe_license_from_file(self, license_file='LICENSE.md'):
            ...
            with open(license_file) as lic:
                ...
                if "MIT License" in lic_data:
                    return {
                        ...
                        'MIT',
                        'remote_url':
                        '%s/%s' % (licenses_provider_URL, 'mit'),
                        'url':
                        '%s/blob/%s/%s' %
                        (self.git_remote_origin_url, self.git_active_branch,
                         license_file)
                    }
                else:
                    print(
                        "Recipe License file {license_file} is not recognized.".
                        format(license_file=license_file))
        else:
            print(
                "Recipe License file {license_file} missing. Adding a default MIT license.".
                format(license_file=license_file))
            return {
                'license':
                'MIT',
                'url':
                '%s/blob/%s/%s' % (self.git_remote_origin_url,
                                   self.git_active_branch, license_file)
            }
        return {}

    def prepare(self):
        self.recipe_license = self.get_recipe_license_from_file()
        if not self.recipe_license:
            self.recipe_license = self.get_recipe_license_from_file('LICENSE')

        self.nameSpace = {
            'name': self.getConanfileVar('name'),
    ...
    def run(self, template='README.md.tmpl', output='README.md'):
        ...
        try:
            ...
            readme_parsed = Template(
                template_data, searchList=[self.nameSpace])
